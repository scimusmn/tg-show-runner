...
import $ from 'jquery';
import {
  Sparklines, SparklinesLine, SparklinesSpots, SparklinesBars, SparklinesReferenceLine,
...
export default class Visualization extends Component {
  ...
  componentDidMount() {
    ...
    if (this.props.trainMode == 'enter') {
      ...
      TweenMax.to(this.trainLightsEnter, 0.15, {
        autoAlpha: 0.0, ease: Power2.easeIn, repeat: -1, yoyo: true,
      });
    } else if (this.props.trainMode == 'exit') {
      TweenMax.set(this.trainLightsExit, { autoAlpha: 1.0 });
      TweenMax.to(this.trainLightsExit, 0.15, {
        autoAlpha: 0.0, ease: Power2.easeIn, repeat: -1, yoyo: true,
      });
    }
  ...
  vistaColorKey() {
    ...
    TweenMax.set(vistaFriendly.target, {
      x: 25, y: -4, scale: this.vistaAdultScale, autoAlpha: 1.0,
    });
    TweenMax.set(vistaUnfriendly.target, {
      x: 25, y: 61, scale: this.vistaAdultScale, autoAlpha: 1.0,
    });
  }
  ...
  createEntrance(vista, index) {
    TweenMax.set(vista.target, {
      x: this.entrancePoint.x, y: this.entrancePoint.y, scale: this.vistaAdultScale, autoAlpha: 1.0,
    });
    ...
    const bezTween = new TweenMax(vista.target, 1.1, {
      bezier: {
        type: 'soft',
        values: [{
          x: this.entrancePoint.x, y: this.entrancePoint.y, x: this.visCenter.x, y: this.entrancePoint.y,
        }, { x: cell.pixelX, y: cell.pixelY }],
        autoRotate: false,
      },
      delay: 0.0,
  ...
  generationSequence() {
    ...
    for (let i = 0; i < this.currentGeneration.length; i += 2) {
      ...
      if (i >= this.currentGeneration.length - 1) {
        const singletonDeath = new TweenMax(this.currentGeneration[i].target, 0.9 * this.timeScale, {
          delay: 2.5 * this.timeScale, ease: Power2.easeOut, autoAlpha: 0.0, scaleY: 0.01, scaleX: 0.1, ease: Power2.easeOut, autoAlpha: 0.0, onComplete: this.onDeathComplete, onCompleteParams: [this.currentGeneration[i]],
        });
      ...
      const tween1 = new TweenMax(v1.target, 0.4 * this.timeScale, {
        x: mid1[0], y: mid1[1], ease: Power4.easeIn, yoyo: true, repeat: 1,
      });
      const tween2 = new TweenMax(v2.target, 0.4 * this.timeScale, {
        x: mid2[0], y: mid2[1], ease: Power4.easeIn, yoyo: true, repeat: 1,
      });
      ...
      const death1 = new TweenMax(v1.target, 0.9 * this.timeScale, {
        delay: 2.5 * this.timeScale, ease: Power2.easeOut, autoAlpha: 0.0, scaleY: 0.01, scaleX: 0.1, onComplete: this.onDeathComplete, onCompleteParams: [v1],
      });
      const death2 = new TweenMax(v2.target, 0.9 * this.timeScale, {
        delay: 2.5 * this.timeScale, ease: Power2.easeOut, autoAlpha: 0.0, scaleY: 0.01, scaleX: 0.1, onComplete: this.onDeathComplete, onCompleteParams: [v2],
      });
  ...
  createBirthAnim(vista, parentsX, parentsY, index, parentCellRef) {
    ...
    const tweenToGrid = new TweenMax(vista.target, 0.6 * this.timeScale, {
      delay: 0.05 * this.timeScale, x: birthTargetPos.x, y: birthTargetPos.y, ease: Power3.easeOut,
    });

    const tweenGrow = new TweenMax(vista.target, 1.0 * this.timeScale, {
      delay: 1.4 * this.timeScale, autoAlpha: 1.0, x: cell.pixelX, y: cell.pixelY, scale: this.vistaAdultScale, ease: Power2.easeInOut,
    });
  ...
  vistasExit() {
    ...
    for (let i = this.currentGeneration.length - 1; i >= 0; i--) {
      if (i < numToExit) {
  ...
  activateVista(friendliness, generation) {
    ...
    if (friendliness > 0.4 && friendliness < 0.65) {
      ...
      TweenMax.to(newVista.target.find('img'), Math.random() * 0.3 + 0.3, {
        scale: 1.1, ease: Power2.easeInOut, repeat: -1, yoyo: true,
      });
    } else if (friendliness >= 0.65) {
      TweenMax.set(newVista.target.find('img'), { scale: 0.95 });
      TweenMax.to(newVista.target.find('img'), Math.random() * 0.2 + 0.2, {
        scale: 1.3, ease: Power2.easeInOut, repeat: -1, yoyo: true,
      });
    }
  ...
  activateHeart(x, y) {
    ...
    const setTween = new TweenMax(heartToActivate.target, 0.0 * this.timeScale, {
      scale: 0.3, autoAlpha: 0.0, x: x + 51, y: y + 65, transformOrigin: 'center center',
    });
    const inTween = new TweenMax(heartToActivate.target, 0.3 * this.timeScale, {
      scale: 0.7, autoAlpha: 1.0, y: '-=30', ease: Power2.easeOut,
    });
    const outTween = new TweenMax(heartToActivate.target, 0.8 * this.timeScale, {
      delay: 0.9 * this.timeScale, autoAlpha: 0.0, ease: Power2.easeIn, onComplete: this.onHeartAnimComplete, onCompleteParams: [heartToActivate],
    });
  ...
  updateGenerationSpeed(generationSpeed, setInitial) {
    ...
    if (setInitial == true) {
      ...
      if (generationSpeed >= 0.5) {
        ...
        TweenMax.to([this.refs.genSpeedBar, this.refs.genSpeedBarLabel], 0.24, {
          autoAlpha: 0.99, ease: Power2.easeInOut, repeat: -1, yoyo: true,
        });
      }
    }
  }
  ...
  sortByFriendliness() {
    ...
    this.currentGeneration.sort((a, b) => ((a.friendliness < b.friendliness) ? 1 : ((b.friendliness < a.friendliness) ? -1 : 0)));
  ...
  createSortAnim(element, sortedX, sortedY) {
    const bezTween = new TweenMax(element, 0.4, {
      scale: 0.2,
      bezier: {
        type: 'soft',
  ...
  createOblivionAnim(element) {
    const tween = new TweenMax(element, 0.5, { autoAlpha: 0.0, y: '+=20', ease: Power2.easeIn });

    return tween;
  }

  createOblivionAnim(vista) {
    const tween = new TweenMax(vista.target, 0.4, { autoAlpha: 0.0, y: '+=30', ease: Power2.easeIn });
  ...
  loadInitialState() {
    ...
    const storedState = JSON.parse(window.localStorage.getItem('visualization-state'));
...
PASSED
        ...
        <h3 className="label speed">
          <span ref="genSpeedBarLabel" className="speedNum">{this.getGenSpeedPercent()}</span>
          <br />
GENERATION
          <br />
SPEED
        ...
        <p className="label averageFriendliness">
AVERAGE FRIENDLINESS:
          <span className="data-val">
            {Math.floor(this.state.averageFriendliness * 100) }
%
          </span>
        </p>
        <p className="label totalVistasSpawned">
TOTAL VISTAS SPAWNED:
          <span className="data-val">{this.state.totalVistasSpawned}</span>
        </p>

        <h3 className="label systemState" ref="systemState">
[
        ...
        <div className="sparkline-container">
          <Sparklines data={this.friendlinessData} limit={25} min={0.0} max={1.0}>
            <SparklinesLine style={{
              strokeWidth: 4, stroke: 'red', fill: 'red', fillOpacity: '0.325',
            }}
            />
